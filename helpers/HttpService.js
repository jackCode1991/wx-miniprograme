...
class HttpService extends WxRequest {
	constructor(options) {
		...
		this.$$path = {
			...
			wechatSignIn: '/user/wechat/wxLogin',
			decryptData : '/user/wechat/decrypt/data',
			signIn      : '/user/sign/in',
			signOut     : '/user/sign/out',
			banner      : '/banner', 
			classify    : '/miniPrograme/category', 
			goods       : '/miniPrograme/category/goodsList', 
			goodsDetail : '/weChatMall/product/goodsDetail',
			search      : '/goods/search/all', 
			queryCarProduct: '/weChatMall/queryCarProduct', 
			address     : '/address', 
			order       : '/order', 
			cart        : '/cart'
        }
	...
	getClassify(params) {
		return this.postRequest(this.$$path.classify, {
			data:params
		})
	}

	getDetail(id) {
		return this.getRequest(`${this.$$path.goodsDetail}/${id}`)
	}
	...
	getCartByOpenId(openId) {
		return this.postRequest(`${this.$$path.queryCarProduct}/${openId}`)
	}
}
